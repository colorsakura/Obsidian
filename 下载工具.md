...
## [Aria2]([aria2](https://aria2.github.io/))

一款支持多种协议的多线程、命令行下载工具，通过`json-rpc` 提供界面操作。支持全平台，不推荐用于 PT 下载，不被多数PT站点接受。
### 配置
...
# 从会话文件中读取下载任务
input-file=C:/Users/iFlygo/.config/aria2/aria2.session

# 会话文件保存路径
# Aria2 退出时或指定的时间间隔会保存`错误/未完成`的下载任务到会话文件
save-session=C:/Users/iFlygo/.config/aria2/aria2.session
...
dht-file-path=C:/Users/iFlygo/.config/aria2/dht.dat

# IPv6 DHT 文件路径，默认：$HOME/.aria2/dht6.dat
dht-file-path6=C:/Users/iFlygo/.config/aria2/dht6.dat
...
```shell
### 提前在 aria2 目录下创建 aria2.session 文件
aria2c --conf-path=C:/Users/<用户名>/.config/aria2/aria2.conf --enable-rpc

### Linux 推荐使用 service 运行
```
## qBittorrent
